...
Deploy gateway-poc to Netto (100.80.187.3).
...
# Deploy
scp target/aarch64-unknown-linux-gnu/release/gateway-poc avero@100.80.187.3:/tmp/
ssh avero@100.80.187.3 "sudo systemctl stop gateway-poc && cp /tmp/gateway-poc /opt/avero/gateway-poc-bin && sudo systemctl start gateway-poc"

# Verify
ssh avero@100.80.187.3 "sudo journalctl -u gateway-poc --since '10 seconds ago' --no-pager | grep gateway_poc_starting"
```
